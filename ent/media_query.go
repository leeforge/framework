...
func (_q *MediaQuery) QueryFormats() *MediaFormatQuery {
	query := (&MediaFormatClient{config: _q.config}).Query()
...
func (_q *MediaQuery) Clone() *MediaQuery {
	...
	return &MediaQuery{
		...
		withFormats: _q.withFormats.Clone(),
...
// WithFormats tells the query-builder to eager-load the nodes that are connected to
// the "formats" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *MediaQuery) WithFormats(opts ...func(*MediaFormatQuery)) *MediaQuery {
	query := (&MediaFormatClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withFormats = query
	return _q
}
...
func (_q *MediaQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Media, error) {
	...
	if query := _q.withFormats; query != nil {
		if err := _q.loadFormats(ctx, query, nodes,
			func(n *Media) { n.Edges.Formats = []*MediaFormat{} },
			func(n *Media, e *MediaFormat) { n.Edges.Formats = append(n.Edges.Formats, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *MediaQuery) loadFormats(ctx context.Context, query *MediaFormatQuery, nodes []*Media, init func(*Media), assign func(*Media, *MediaFormat)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(mediaformat.FieldMediaID)
	}
	query.Where(predicate.MediaFormat(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(media.FormatsColumn), fks...))
	}))
